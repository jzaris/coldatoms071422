...
int main(int argc, char *argv[]){
	...
	if(argc == 3){
		...
		std::ifstream src(argv[2], std::ios::binary);
	        ...
	        std::string prev_name(argv[2]);
		int dot_index = prev_name.find_first_of('.');
		new_name_str =  prev_name.substr(0,dot_index);
		new_name_str.append("1.dat");

		std::ofstream dst(new_name_str, std::ios::binary);
		dst << src.rdbuf();
		...
		//char *line = NULL;
	        //size_t len = 0;
		f = fopen(new_name, "r");  /*Open previous output file to set positions and velocities*/
		if (f == NULL)
                	exit(EXIT_FAILURE);
		...
		int this_char;
		double this_out_val;
		while(spaces <= particles_global*6-1){
		...
		while(vals_read < particles_global*6){
			fscanf(f, "%lf", &this_out_val);
			data_type = vals_read/particles_global;
			particle_index = vals_read%particles_global;
			arr(data_type,particle_index) = this_out_val;
			vals_read+=1;
			cout << std::scientific << this_out_val << "\n";
		}
		fseek(f, 0, SEEK_SET);
		double this_phi;
		int doubles_read = 0;
...
void coulomb_force_fmm()
{
	...
	static double arr_trans[100000*3];
	for (int j = 0; j<3;j++){
		for (int i =0; i<particles_global;++i){
